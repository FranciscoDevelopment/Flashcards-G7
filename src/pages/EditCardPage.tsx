...
export default function EditCardPage() {
  ...
  const { decks, cards, updateCard } = useDeckStore((state) => ({
    decks: state.decks,
    cards: state.cards,
  ...
  // Load card contents on mount or whenever card is found
  useEffect(() => {
    if (card) {
      setFront(card.front);
      setBack(card.back);
  ...
  if (!deck || !card) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center text-slate-100 bg-slate-950">
        <h3 className="text-xl font-bold">Elemento no encontrado</h3>
        <p className="text-slate-400 mt-2">No se pudo encontrar el mazo o la tarjeta seleccionada.</p>
    ...
    <div className="flex-1 overflow-y-auto p-6 md:p-10 space-y-8 bg-slate-950 text-slate-100 flex flex-col max-w-5xl mx-auto w-full">
      ...
      <div className="space-y-4 shrink-0">
        ...
        <div className="border-b border-slate-900 pb-6">
          ...
          <h2 className="text-3xl font-extrabold text-white tracking-tight mt-1">Editar Tarjeta</h2>
          <p className="text-slate-400 mt-1">Edita el contenido del anverso y reverso de esta tarjeta de memoria.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 lg:col-span-7 bg-slate-900/10 border border-slate-900 p-6 md:p-8 rounded-3xl">
          <div className="space-y-2">
            ...
            <textarea
              ...
              placeholder="Ej: ¿Cuál es la complejidad de QuickSort en el peor caso?"
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="flex items-center justify-between text-xs text-slate-505 font-semibold uppercase tracking-wider">
          ...
          <div className="w-full aspect-[4/3] rounded-3xl border border-slate-900 bg-slate-900/10 p-8 flex flex-col justify-between shadow-xl relative overflow-hidden">
            ...
            <div className="flex justify-between items-center text-[9px] text-slate-600 font-mono border-t border-slate-900/60 pt-3 z-10">
              <span>Tarjeta #{card.id.split('-').pop()}</span>
          ...
          <div className="p-4 rounded-2xl bg-slate-900/20 border border-slate-900 text-xs text-slate-400 leading-normal flex gap-2">
            ...
            <span>
              Editar esta tarjeta actualizará su contenido de inmediato sin reiniciar su nivel de **Caja Leitner** ({card.box}).
            </span>
          </div>
        </div>
      </div>
